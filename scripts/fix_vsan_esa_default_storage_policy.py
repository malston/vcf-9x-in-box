...
class VSANPolicyFixer:
    ...
    def wait_for_ping(self, timeout: int = 1800) -> bool:
        ...
        print(
            f"{Colors.YELLOW}⏳ Checking if vCenter ({self.vcenter['hostname']}) is pingable...{Colors.NC}"
        )
        ...
        while elapsed < timeout:
            ...
            if elapsed > 0:
                print(
                    f"{Colors.YELLOW}⏳ vCenter not pingable yet. "
        ...
        print(
            f"{Colors.RED}ERROR: Timeout waiting for vCenter to be pingable{Colors.NC}"
        )
        return False
    ...
    def wait_for_vcenter_connection(self, timeout: int = 1800) -> bool:
        ...
        print(
            f"{Colors.YELLOW}⏳ Waiting for vCenter to accept connections...{Colors.NC}"
        )
        ...
        while elapsed < timeout:
            try:
                ...
                if elapsed > 0:
                    print(
                        f"{Colors.YELLOW}⏳ vCenter not ready for login yet. "
                        f"Sleeping for {interval // 60} minutes... "
                        f"(elapsed: {elapsed // 60}/{timeout // 60} min){Colors.NC}"
                    )
                time.sleep(interval)
    ...
    def wait_for_vcf_policy(self, timeout: int = 1800) -> bool:
        ...
        print(
            f"{Colors.YELLOW}⏳ Waiting for VCF Storage Policy to be available...{Colors.NC}"
        )
        ...
        while elapsed < timeout:
                ...
                if profile_ids:
                    ...
                    for profile in profiles:
                        if "VCF" in profile.name:
                            print(
                                f"{Colors.GREEN}✓ VCF Storage Policy found: {profile.name}{Colors.NC}\n"
                            )
            ...
            if elapsed > 0:
                print(
                    f"{Colors.YELLOW}⏳ VCF Storage Policy not found yet. "
    ...
    def _connect_spbm(self):
        ...
        try:
            ...
            pbm_stub = pbm.VmomiSupport.GetStub(
                "pbm", host=hostname, sslContext=context, connectionPoolTimeout=0
            )
    ...
    def fix_storage_policy(self, dry_run: bool = False) -> bool:
        ...
        try:
            ...
            if dry_run:
                print(
                    f"{Colors.YELLOW}DRY RUN: Would update VCF Storage Policy FTT from 1 to 0{Colors.NC}\n"
                )
                return True
            ...
            rule = pbm.capability.Rule(
                capabilityId=capability.id, constraint=[capability.constraint[0]]
            )

            rule_set = pbm.capability.RuleSet(rule=[rule])
                ...
                constraints=pbm.profile.SubProfileCapabilityConstraints(
                    subProfiles=[
                        pbm.profile.SubProfileCapabilityConstraints.SubProfile(
                            name="VSAN", capability=[capability]
                        )
                    ]
                )
            )
            ...
            # Update the profile
            pm.PbmUpdate(profileId=vcf_profile.profileId, updateSpec=spec)

            print(
                f"{Colors.GREEN}✓ VCF Storage Policy updated successfully (FTT: 1 → 0){Colors.NC}\n"
            )
            return True
    ...
    def run(self, dry_run: bool = False, skip_wait: bool = False) -> bool:
        ...
        if self.host_count >= 3:
            print(
                f"{Colors.YELLOW}⚠ WARNING: This script is only needed for 2-node deployments{Colors.NC}"
            )
            print(
                f"{Colors.BLUE}Your configuration has {self.host_count} hosts.{Colors.NC}"
            )
            print(
                f"{Colors.BLUE}The default vSAN policy (FTT=1) is correct for 3+ node deployments.{Colors.NC}\n"
            )
